...
# ======================================================
# ACE v7.0 — ACC Core + Lite-Core-Network (Terminal, GPU/MPS)
# ------------------------------------------------------
...
def ace_once(prompt: str, mem: Dict[str, Any]) -> str:
    ...
    if not acw_enabled:
        ...
        return generate_text(
            base_prompt,
            temperature=0.6,   # slightly lower temp = fewer hallucinations
            top_p=0.9,
    ...
    if story_mode:
        if continuation and last_story:
            ...
            story_prompt = build_story_prompt(clean_prompt, hopeful)
        ...
        _ = hallucination_level(state, h_score, story_mode=True)  # tracked, not forced
...
def main() -> None:
    ...
    print("========================================")
    print(" ACE v7.0 — ACC Core + Lite-Core-Network (Terminal)")
    ...
    while True:
        ...
        # Debug: Show ACW status for this prompt
        _, acw_enabled = split_acw_marker(user)
        if acw_enabled:
            print("[DEBUG] ACE is processing (ACW enabled)")
        else:
            print("[DEBUG] Raw model (ACW disabled)")

        response = ace_once(user, mem)
        save_mem(mem)
